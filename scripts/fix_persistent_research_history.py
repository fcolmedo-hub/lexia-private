...
SERVER_HELPER_MARK = "LEXIA_UI2_PERSISTENT_RESEARCH_HISTORY_20260829"
INDEX_MARK = "LEXIA_UI2_PERSISTENT_RESEARCH_HISTORY_FRONTEND_20260829"
...
SERVER_HELPER = r'''# LEXIA_UI2_PERSISTENT_RESEARCH_HISTORY_20260829
def _research_history_items(limit: int = 12):
    ...
    try:
        ...
        for row in rows:
            ...
            facts = str(row["facts"] or "").strip() if "facts" in row.keys() else ""
            objective = str(row["objective"] or "").strip() if "objective" in row.keys() else ""
            instruction = (
                str(row["additional_instruction"] or "").strip()
                if "additional_instruction" in row.keys()
                else ""
            )
            ...
            items.append({
                ...
                "maxSources": (
                    ...
                    if "max_sources" in row.keys() and row["max_sources"] is not None
                    else None
                ),
                "createdAt": (
...
NEW_FRONTEND = r'''  /* LEXIA_UI2_PERSISTENT_RESEARCH_HISTORY_FRONTEND_20260829 */
...
def patch_server() -> None:
    ...
    if SERVER_HELPER_MARK not in text:
        if SERVER_HELPER_ANCHOR not in text:
            raise SystemExit("No encontré el ancla _record_ui2_search_history en server.py. No modifiqué archivos.")
        text = text.replace(SERVER_HELPER_ANCHOR, SERVER_HELPER + SERVER_HELPER_ANCHOR, 1)

    if 'path == "/api/research-history"' not in text:
        if SERVER_ROUTE_ANCHOR not in text:
            raise SystemExit("No encontré do_GET/maintenance-live en server.py. No modifiqué archivos.")
    ...
    if text != original:
        ...
        print("OK: backend de historial persistente aplicado en server.py")
    else:
...
def patch_index() -> None:
    ...
    if OLD_FRONTEND not in text:
        raise SystemExit("No encontré el bloque actual de historial de Investigación en index.html. No modifiqué index.html.")
    ...
    print("OK: frontend de historial persistente aplicado en index.html")


def main() -> None:
    if not SERVER.exists() or not INDEX.exists():
        raise SystemExit("No encontré app/ui2/server.py o app/ui2/index.html")
    patch_server()
    patch_index()
    print("OK: Consultas recientes de Investigación ahora se reconstruyen desde SQLite al iniciar LexIA.")

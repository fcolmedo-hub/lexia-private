...
(function () {
  ...
  function style() {
    ...
    const css = [
      ...
      '#searchpage #realSearchResults .result-actions{width:auto!important;min-width:0!important;flex-direction:row!important;align-items:center!important;justify-content:flex-end!important;gap:4px!important}#searchpage #realSearchResults .result-actions .search-investigate-file,#searchpage #realSearchResults .result-actions .search-file-info,#searchpage #realSearchResults .result-actions .search-delete-file,#searchpage #realSearchResults .result-actions [data-lexia-case-link]{width:28px!important;min-width:28px!important;height:28px!important;min-height:28px!important;padding:0!important;display:grid!important;place-items:center!important;border-radius:7px!important;font-size:0!important;line-height:1!important}#searchpage #realSearchResults .result-actions .search-investigate-file::before{content:"⌕";font-size:17px;line-height:1;font-weight:800}#searchpage #realSearchResults .result-actions .search-file-info::before{content:"i";font-size:14px;line-height:1;font-family:Georgia,serif;font-weight:800}#searchpage #realSearchResults .result-actions .search-delete-file::before{content:"×";font-size:18px;line-height:1;font-weight:700}#searchpage #realSearchResults .result-actions [data-lexia-case-link]{border-color:#169b55!important;background:#169b55!important;color:#fff!important}#searchpage #realSearchResults .result-actions [data-lexia-case-link]::before{content:"+";font-size:18px;line-height:1;font-weight:800}',
  ...
  function revealExistingHighlight(reader, savedText) {
    ...
    if (!exact || !reader.firstChild) return false;
    ...
    range.setStart(reader.firstChild, start); range.setEnd(reader.firstChild, start + length);
  ...
  function openEvidenceDialog(snapshot, node, block, documents, existing) {
    ...
    const dialog = el('dialog', {className: 'lexia-evidence-dialog evidence-selection-dialog'}), select = el('select', {className: 'workspace-enunciado'}), reader = el('pre', {className: 'evidence-reader', textContent: 'Elegí un documento para cargar su texto indexado.'}), status = el('p', {className: 'evidence-selection-status', textContent: 'Seleccioná con el mouse el pasaje exacto que querés conservar.'});
    ...
    const close = el('button', {type: 'button', className: 'cases-button-secondary', textContent: 'Cerrar'}), save = el('button', {type: 'button', className: 'cases-button', textContent: existing ? 'Reemplazar resaltado' : 'Incorporar resaltado'});
    ...
    const load = async () => {
      ...
      try {
        ...
        if (existing) requestAnimationFrame(() => {
          if (revealExistingHighlight(reader, existing.selected_text)) {
            status.textContent = 'Este es el pasaje actualmente incorporado. Podés seleccionar otro texto y reemplazarlo.';
          } else {

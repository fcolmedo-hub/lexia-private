...
(function () {
  ...
  function style() {
    ...
    const css = [
      ...
      '.cases-button,.cases-button-secondary,.cases-icon{font:inherit;font-size:11px;font-weight:800;cursor:pointer;border-radius:9px;padding:10px 12px}.cases-button{background:#5146f6;color:#fff;border:1px solid #5146f6}.cases-button:hover{background:#4136df}.cases-button-secondary{background:#fff;color:#465176;border:1px solid #d8deed}.cases-button-secondary:hover{border-color:#6459f4;color:#493de2}.cases-icon{background:transparent;color:#5f6989;border:0;padding:6px 7px}.cases-icon:hover{background:#f0efff;color:#493de2}.cases-danger{border-color:#f0c7ce;color:#b23848}.cases-danger:hover{background:#fff4f5;border-color:#df7786;color:#9d2939}',
  ...
  function newCaseForm() {
    ...
    const name = el('input', {placeholder: 'Carátula o nombre del caso', required: 'required'}), authority = el('input', {placeholder: 'Juzgado, tribunal o autoridad'}), fileNumber = el('input', {placeholder: 'Número de expediente'}), parties = el('input', {placeholder: 'Partes o intervinientes'}), description = el('textarea', {placeholder: 'Resumen del caso: hechos, pretensión y estado actual.'});
    ...
    form.append(el('div', {className: 'cases-form-grid'}, field('Carátula', name, true), field('Tribunal o autoridad', authority), field('Expediente', fileNumber), field('Partes', parties, true), field('Resumen del caso', description, true)), el('div', {className: 'cases-form-actions'}, cancel, submit));
    form.addEventListener('submit', async event => {
      event.preventDefault(); submit.disabled = true;
      try { const response = await api('/api/cases', {method: 'POST', body: JSON.stringify({name: name.value, authority: authority.value, file_number: fileNumber.value, parties: parties.value, description: description.value})}); currentCase = response.case; showNewCase = false; await loadCases(false); }
  ...
  function editCase(snapshot) {
    ...
    form.append(el('div', {className: 'cases-form-grid'}, field('Carátula', name, true), field('Tribunal o autoridad', authority), field('Expediente', fileNumber), field('Partes', parties, true), field('Resumen del caso', description, true)), el('div', {className: 'cases-form-actions'}, cancel, save));
    form.addEventListener('submit', async event => {
      event.preventDefault(); save.disabled = true;
      try { const response = await api('/api/cases/update', {method: 'POST', body: JSON.stringify({case_id: details.id, name: name.value, authority: authority.value, file_number: fileNumber.value, parties: parties.value, description: description.value})}); currentCase = response.case; editingCase = false; await loadCases(false); }
  ...
  function primary(snapshot, node) {
    const article = el('article', {className: 'primary-branch'}), title = el('div', {className: 'branch-title'}, el('b', {textContent: node.title}), el('small', {textContent: node.primary_document_name || 'Hito sin documento inicial'}));
    ...
    article.append(el('header', {className: 'primary-head'}, el('span', {className: 'branch-mark', textContent: '↳'}), title, el('div', {className: 'branch-actions'}, addQuestion, edit, remove)));

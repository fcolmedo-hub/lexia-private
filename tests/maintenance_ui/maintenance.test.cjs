...
async function setup(){
  ...
  let deleted='';
  const snapshot=()=>({ok:true,live:{autosync:{phase:'idle'},ocr:{running:false,pending:2,error:1},catalog:{documents:4}},autosync_config:{mode:'manual'}});
  ...
  return {window,document,requests,opened,confirmations,click,select,
    approve:()=>approve=true,failDuplicates:()=>duplicateError=true,setRows:value=>rows=value};
}
...
test('OCR selections persist through refresh and send only the selected paths',async()=>{
  const app=await setup();await app.click('[data-maint-tab="automation"]');await app.click('[data-ocr-filter="pending"]');
...
test('Opening and reprocesing an individual error use that exact file',async()=>{
  const app=await setup();await app.click('[data-maint-tab="automation"]');await app.click('[data-ocr-filter="error"]');
...
test('OCR pages reach records after 100 and filter changes clear hidden selections',async()=>{
  ...
  await app.click('[data-maint-tab="automation"]');await app.click('[data-ocr-filter="pending"]');

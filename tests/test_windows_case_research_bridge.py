...
def test_counterpart_is_only_adversarial_context():
    ...
    assert "resolved?.node?.adversary_text" in bridge
    assert "counterpartText(article,base.blockIndex,base)" in bridge
    assert "PLANTEO DE LA CONTRAPARTE:" in bridge
    assert "fillCaseInstructions(instructions,counter)" in bridge
    assert "node.scrollTop=0" in bridge

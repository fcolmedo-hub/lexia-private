...
def main():
    ...
    sp=resolve_under_root(root, SETTINGS.autosync_state_path)
    cp=resolve_under_root(root, SETTINGS.catalog_path)
    op=resolve_under_root(root, SETTINGS.ocr_queue_path)

    print("# LEXIA — DIAGNOSTICO ESTADO AUTOSYNC v1.1 (SOLO LECTURA)")
    print("Raiz LexIA:", root)
    ...
    if op.exists():
        ...
        try:
            ...
            for t in tabs:
                cs=[r["name"] for r in c.execute("pragma table_info("+t+")")]
                if "status" in [x.lower() for x in cs]:
                    ocr_table=(t,cs)
                    break
    ...
    print("  last_sync:",state.get("last_sync","") or "[vacio]")
    print()

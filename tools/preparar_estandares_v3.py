...
DEFAULT_PROMPT = REPO_ROOT / "prompt" / "standards_extraction_v3.txt"
DEFAULT_OUTPUT = REPO_ROOT / "runtime" / "standards_pilot" / "export_50_v3"


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Prepara prompts v3: mismo texto jurídico de v2 y esquema de salida compacto."
    )
    ...
    if not args.prompt.exists():
        parser.error(f"No existe el prompt v3: {args.prompt}")
    ...
    manifest = {
        ...
        "compact_positional_output": True,
        "coded_attribution_values": True,
        "python_expands_final_schema": True,
        "note": (
            "Mantiene el mismo texto jurídico completo de v2. El ahorro adicional proviene "
            "del formato de salida: arrays posicionales y códigos cortos, sin perder campos semánticos."
        ),
    }
    ...
    print(f"Preparación v3 lista en: {args.output.resolve()}")
    return 0
